...
return false;}
function parse(object,args){var params;if(!replaceEmoji||!twemoji||!object||('string'!==typeof object&&(!object.childNodes||!object.childNodes.length))){return object;}
args=args||{};params={base:settings.baseUrl,ext:settings.ext,className:args.className||'emoji',callback:function(icon,options){switch(icon){case'a9':case'ae':case'2122':case'2194':case'2660':case'2663':case'2665':case'2666':return false;}
if(!settings.supports.flag&&settings.supports.simple&&settings.supports.unicode8&&!/^1f1(?:e[6-9a-f]|f[0-9a-f])-1f1(?:e[6-9a-f]|f[0-9a-f])$/.test(icon)){return false;}
...
if(settings){replaceEmoji=!settings.supports.simple||!settings.supports.flag||!settings.supports.unicode8;if(settings.DOMReady){load();}else{settings.readyCallback=load;}}
return{replaceEmoji:replaceEmoji,parse:parse,test:test};}
